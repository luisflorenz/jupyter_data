...
bmi_mass_index = df.weight /(df.height/100)**2
df['overweight'] = (bmi_mass_index).apply(lambda x: 1 if x >25 else 0)
...
def draw_heat_map():
    # Clean the data
    df_heat = df_cat=df.melt(id_vars=['cardio'],value_vars=sorted(['cholesterol','gluc','smoke','alco','active','overweight']))
